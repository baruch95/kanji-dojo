...
- Publication: app commit `7fbd78c` was pushed to `origin/main` and deployed by manual Pages run [36170351176](https://github.com/baruch95/kanji-dojo/actions/runs/36170351176). Documentation commit `ffc1f99` was then pushed and deployed by [36170798829](https://github.com/baruch95/kanji-dojo/actions/runs/36170798829). GitHub passed clean `npm ci`, typecheck, lint, unit tests, data validation, Chromium/WebKit, offline/update tests, production subpath build, artifact upload, and deploy. The live URL <https://baruch95.github.io/kanji-dojo/> displayed 50 new cards and “Available offline” after reload. Live HTML, manifest, service worker, and 192/512 icons returned HTTP 200; manifest scope and start URL were `/kanji-dojo/`. This verifies publication and desktop browser readiness, not physical iPad behavior or release gates.

## Post-M7 owner feedback — local change, 2026-09-25

- Owner reports the published 50% accuracy setting is too strict for handwriting and 10% is usable. No attempts or counts were supplied. `m2-provisional-2` moves the 50% soft thresholds to the old 10% values; hard count/order/direction checks are unchanged. Synthetic tests cover a modest shifted attempt now passing at 50, gross displacement failing, all canonical references, and reversal/swap/count failures. This is usability feedback, not measured calibration (Q-004, Q-006).
- The primary action changes label in place from Check to Continue, Start learning, Finish session, or Retry save after submission. The result feedback remains below the controls. An active session saved under the old matcher version ends safely after the update; card progress remains.
- Local checks under pinned Node 24.21.0: `npm run typecheck`, `npm run lint`, `npm run test` (26 tests in 8 files), `npm run data:validate`, `npm run test:e2e` (16 Chromium/WebKit cases), `npm run test:pwa` (root and repository-path offline reopening plus safe update), `npm run build`, and `git diff --check` passed. The browser case confirms the action retains its document position after Check. No new physical iPad attempt was recorded.
