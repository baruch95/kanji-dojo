...
| D-20 | Owner feedback, 2026-09-25 | Keep the primary action at the Check position when it changes to Continue, Start learning, Finish session, or Retry save. The owner reports that 50% geometric accuracy is too strict and 10% is usable, so `m2-provisional-2` makes the new default midpoint equal to the former 10% limits. Hard count/order/direction rules stay fixed. Existing active sessions end safely on matcher-version change; saved progress remains. Quantitative calibration is still deferred. | product-spec.md; handwriting-spec.md; matcher-calibration.md; Q-006 |
